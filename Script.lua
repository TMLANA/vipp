...
redis:del(boss..":RtbaNew9:"..msg.chat_id_)
...
local Rtba1 = redis:get(boss..":RtbaNew9:"..msg.chat_id_) or " لايوجد "
...
return sendMsg(msg.chat_id_,msg.id_,"المالك :\n["..v.user.first_name.."](t.me/"..(v.user.username or "bossK"))
end
end
...
elseif redis:sismember(boss..':MATH_BOT:',UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"• لا يمكنك تقييد Myth 🎖\n")
...
if UserID == our_id then return sendMsg(arg.ChatID,arg.MsgID,"• لا يمكنك تنفيذ الامر بالرد ع رسالة البوت \n") end
GetUserID(UserID,function(arg,data)
if msg.MudoUser then redis:srem(boss..'gban_users',arg.UserID)  end 
Restrict(arg.ChatID,arg.UserID,2)
...
local UserID = data.id_
if UserID == our_id then return sendMsg(arg.ChatID,arg.MsgID,"• لا يمكنك تنفيذ الامر بالرد ع رسالة البوت \n") end
if msg.MudoUser then redis:srem(boss..'gban_users',UserID)  end 
Restrict(arg.ChatID,UserID,2)
...
elseif MsgText[2] and MsgText[2]:match('^%d+$') then
if msg.MudoUser then redis:srem(boss..'gban_users',MsgText[2])  end 
...
if MsgText[1] == "قفل ردود MY" 		then return lock_reda(msg) end
...
if MsgText[1] == "فتح ردود MY" 		then return unlock_reda(msg) end 
...
if MsgText[1] == "مسح رسائلي" or MsgText[1] == "مسح رسايلي"  then
local msgs = redis:get(boss..'msgs:'..msg.sender_user_id_..':'..msg.chat_id_) or 1
if msgs == 0 then  return "• ** لا يوجد رسائل لك في البوت  ." end
...
if redis:get(boss.."chat:tagall"..msg.chat_id_) then  return "• يمكنك عمل منشن كل *5 دقائق* فقط" end 
...
return '•  عدد جهاتك المضافه » 【'..(redis:get(boss..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)..'】 . \n🐾'
end
...
if not msg.SudoBase then return "• هذا الامر يخص 〚 Dev 〛 فقط  \n" end
local mtwren = redis:scard(boss..':MATH_BOT:')
if mtwren == 0 then  return "• عذرا لا يوجد MY في البوت  " end
redis:del(boss..':MATH_BOT:') 
return "• تم مسح 〚 * "..mtwren.." * 〛 من قائمه MY \n"
end

if MsgText[1] == 'مسح قائمه العام'  then
if not msg.MudoUser then return"• هذا الامر يخص 〚 Dev,Myth 🎖 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_blo') then 
return "• اوامر مسح العام مقفول من قبل Dev  " end
...
if redis:sismember(boss..':MATH_BOT:',UserID) then 
...
redis:srem(boss..':MATH_BOT:',UserID)
...
RUSERNAME = ResolveUserName(data)
NameUser = Hyper_Link_Name(data)
if redis:sismember(boss..':MATH_BOT:',arg.UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"• المستخدم  ⇦「 "..NameUser.." 」 \n• تم بالتأكيد رفعه Myth 🎖 في البوت") 
else
sendMsg(arg.ChatID,arg.MsgID,"• المستخدم  ⇦「 "..NameUser.." 」 \n• تم رفعه Myth 🎖 في البوت") 
redis:hset(boss..'username:'..arg.UserID,'username',RUSERNAME)
redis:sadd(boss..':MATH_BOT:',arg.UserID)
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
end
...
redis:hset(boss..'username:'..UserID,'username',ReUsername)
redis:sadd(boss..':MATH_BOT:',UserID)
sendMsg(arg.ChatID,arg.MsgID,"• المستخدم  ⇦「 "..NameUser.." 」 \n• تم رفعه Myth 🎖 في البوت") 
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
end 
...
GetUserID(UserID,function(arg,data)
NameUser = Hyper_Link_Name(data)
if not redis:sismember(boss..':MATH_BOT:',arg.UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"• المستخدم  ⇦「 "..NameUser.." 」 \n• تم بالتأكيد تنزيله Myth 🎖 في البوت") 
else
sendMsg(arg.ChatID,arg.MsgID,"• المستخدم  ⇦「 "..NameUser.." 」 \n• تم تنزيله Myth 🎖 في البوت") 
redis:srem(boss..':MATH_BOT:',arg.UserID)
end  
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
end
...
local UserID = data.id_
NameUser = Hyper_Link_Name(data)
if not redis:sismember(boss..':MATH_BOT:',UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"• المستخدم  ⇦「 "..NameUser.." 」 \n• تم بالتأكيد تنزيله Myth 🎖 في البوت") 
else
redis:srem(boss..':MATH_BOT:',UserID)
sendMsg(arg.ChatID,arg.MsgID,"• المستخدم  ⇦「 "..NameUser.." 」 \n• تم تنزيله Myth 🎖 في البوت") 
...
sendPhoto(msg.chat_id_,msg.id_,Photo_Weloame,[[• أهلاً بك عزيزي انا بوت اسمي ]]..redis:get(boss..':NameBot:')..[[
...
return'• شرط التفعيل البوت اذا كانت المجموعه اكثر من *【'..redis:get(boss..':addnumberusers')..'】* عضو  \n' 
end 
end

if MsgText[1] == 'المجموعات' or MsgText[1] == "المجموعات " then 
if not msg.MudoUser then return"• هذا الامر يخص 〚 Dev 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_has') then 
return "• عدد المجموعات مقفوله من قبل Dev  " end
...
if not msg.MudoUser then return "• هذا الامر يخص 〚 Dev,Myth 🎖 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss.."lockidedit") then return "• الامر معطل من قبل Dev  \n" end
...
if not msg.MudoUser then return "• هذا الامر يخص 〚 Dev,Myth 🎖 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss.."lockidedit") then return "• تعيين الايدي معطل من قبل Dev  \n" end
...
if not msg.MudoUser then return"• هذا الامر يخص 〚 Dev 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_has') then 
return "• قائمه المجموعات مقفوله من قبل Dev  " end
...
if not msg.SudoUser then return"• هذا الامر يخص 〚 Dev,Myth 🎖,Myth 〛 فقط  \n" end
...
if not msg.MudoUser then return "• هذا الامر يخص 〚 Dev 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_blo') then 
return "• امر حظر العام مقفول من قبل Dev  " end
if not MsgText[2] and msg.reply_id then 
GetMsgInfo(msg.chat_id_,msg.reply_id,function(arg,data)
if not data.sender_user_id_ then return sendMsg(arg.ChatID,arg.MsgID,"• عذرا هذا العضو ليس موجود ضمن المجموعات \n") end
local UserID = data.sender_user_id_
...
if not msg.MudoUser then return"• هذا الامر يخص 〚 Dev 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_blo') then 
return "• اوامر الغاء الحظر العام مقفوله من قبل Dev  " end
...
if not msg.MudoUser then return"• هذا الامر يخص 〚 Dev,Myth 🎖 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_reda') then 
return "• اضافة الردود مقفوله من قبل Dev  " 
end
...
if not msg.MudoUser then return"• هذا الامر يخص 〚 Dev,Myth 🎖 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_reda') then 
return "• اوامر مسح الردود مقفوله من قبل Dev  " 
end
...
if not msg.MudoUser then return"• هذا الامر يخص 〚 Dev,Myth 🎖 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_reda') then 
return "• اوامر الردود مقفوله من قبل Dev  " 
end
...
if not msg.MudoUser then return"• هذا الامر يخص 〚 Dev,Myth 🎖 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_reda') then 
return "• مسح الردود مقفوله من قبل Dev  " 
end
...
if not msg.MudoUser then return"• هذا الامر يخص 〚 Dev,Myth 🎖 〛 فقط  \n" end
if not msg.SudoBase and not redis:get(boss..'lock_reda') then 
return "• الردود المتعدده مقفوله من قبل Dev  " 
end
...
if msg.MudoUser and (MsgText[1]=="م1" or MsgText[1]=="م2" or MsgText[1]=="م3" or MsgText[1]=="م D" or MsgText[1]=="م4" or MsgText[1]=="الاوامر") and redis:get(boss..":Witting_awamr_witting"..msg.chat_id_..msg.sender_user_id_) then 
...
if not redis:get(boss.."lock_leftgroup"..msg.chat_id_) then  return "• امر اطردني معطل من قبل اداره المجموعة  \n" end
...
𝐒𝐨𝐮𝐫𝐜𝐞 𝐉𝐚𝐜𝐤 : 
———————
𝐃𝐞𝐯 : [𝑵𝒆𝒍𝒔𝒐𝒏](t.me/URRRP)
𝐂𝐡𝐚 : [Jack Updates](t.me/KBBBD)
]]
end
...
text = [[• أهلاً بك عزيزي انا بوت اسمي []]..redis:get(boss..':NameBot:')..[[] .
• اختصاص البوت حماية المجموعات
• اضف البوت الى مجموعتك .
• ارفعه مشرف وارسل تفعيل ليتم تفعيل المجموعة .
]]..SUDO_USERR..[[
...
redis:set(boss..":RtbaNew9:"..msg.chat_id_,msg.text)
elseif rtbanamenew == "Myth" then
...
if msg.text:match("^"..Bot_Name.." غادر$") and (msg.SudoBase) then
...
gtupe = 'Myth '
elseif redis:sismember(boss..':MATH_BOT:',msg.adduser) then 
gtupe = 'Myth 🎖 '
...
return false
elseif (msg.text:match("ه‍") or msg.text:match("ی") or msg.text:match("ك")) and redis:get(boss.."lock_pharsi"..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)

...
download_file('https://raw.githubusercontent.com/bandarr009/rozz/main/Run.lua','./inc/Run.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'40% - |40٪       |') end
download_file('https://raw.githubusercontent.com/bandarr009/rozz/main/locks.lua','./inc/locks.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'60% - |انتظر    |') end
download_file('https://raw.githubusercontent.com/bandarr009/rozz/main/Script.lua','./inc/Script.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'80% - |انتظر  |') end
download_file('https://raw.githubusercontent.com/bandarr009/rozz/main/functions.lua','./inc/functions.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'90% - |انتظر   |') end
download_file('https://raw.githubusercontent.com/bandarr009/rozz/main/games.lua','./plugins/games.lua')
...
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(boss..":NameBot:") or "نيلبون"
...
usernn = data.username_ or "URRRP"
else 
Name = data.title_
usernn = data.type_.user_.username_ or "URRRP"
end
Name = Name:gsub('[[][]]','')
...
var = redis:get(boss..":RtbaNew1:"..ChatID) or 'Dev 🎖' 
elseif redis:sismember(boss..':SUDO_BOT:',UserID) then
var = redis:get(boss..":RtbaNew2:"..ChatID) or 'Myth ' 
elseif redis:sismember(boss..':MATH_BOT:',UserID) then
var = redis:get(boss..":RtbaNew9:"..ChatID) or 'Myth 🎖 '
...
var = redis:get(boss..":RtbaNew5:"..ChatID) or 'مدير️' 
...
var = redis:get(boss..":RtbaNew7:"..ChatID) or 'مميز' 
else
var = 'عضو️' 
end
return var
...
message = message ..i.. '-l ['..info.username..'](t.me/URRRP) \n'
end
...
if #list==0 then  message = message.."* لا يوجد Myth  حاليا \n  *"
else
...
message = message ..k.. '- ['..info.username..'](t.me/URRRP) : (`' ..v.. '`)'..count..' \n'
end
end 
end
...
function suddolist(msg)
local list = redis:smembers(boss..':MATH_BOT:')
message = '*•* قائمه MY : \n\n*•* ['..SUDO_USER..'] ➯ (' ..SUDO_ID.. ')'..redis:scard(boss..'mtwr_count'..SUDO_ID)..'\n*ـــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــ*\n'
if #list==0 then  message = message.."* لا يوجد Myth 🎖 حاليا \n  *"
else
for k,v in pairs(list) do
...
message = message ..k.. '-l ['..info.username..'](t.me/URRRP) l : (`' ..v.. '`) \n'
end
end
end
...
message = message ..k.. '• ['..info.username..'](t.me/URRRP) l : (`' ..v.. '`) \n'
end
end 
if utf8.len(message) > 4096 then
return " | لا يمكن عرض المحظورين بسبب القائمه كبيره جدا ."
...
if NumAdmin == 0 then 
return sendMsg(arg.chat_id_,arg.id_,"• لا يوجد أدمنيه لكي يتم رفعهم \n")
else
return sendMsg(arg.chat_id_,arg.id_,"• تم رفع  { *"..NumAdmin.."* } من الادمنيه‌‏ في البوت \n")
end
...
if not msg.SudoUser and not lock_servicez then return '• أنت لست المطور ' end
...
return sendMsg(msg.chat_id_,msg.id_,"| اشترك بالقنا‌‏ه اولا ["..UserChaneel.."] \n ثم ارجع ارسل تفعيل .")
end
else
return "| اشترك بالقنا‌‏ه اولا ["..UserChaneel.."] \n ثم ارجع ارسل تفعيل ."
end
end
...
return sendMsg(arg.chat_id_,arg.id_,'*•* لا يمكنني تفعيل البوت في المجموعه‏ يجب ان يكون اكثر من *【'..GroupUsers..'】* عضو ')
end
...
..' • الايدي : `'..arg.chat_id_..'`\n'
..'• ألاعضاء : *'..arg.Groupcount..'*  \n\n• معلومات الشخص \n'
..'*•* الاسم : ['..FlterName(datai.first_name_..' '..(datai.last_name_ or ""),23)..'](tg://user?id='..arg.sender_user_id_..') \n\n'
...
elseif redis:sismember(boss..':MATH_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"*•* لا يمكنك طرد Myth 🎖\n") 
elseif redis:sismember(boss..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"*•* لا يمكنك طرد Myth\n") 
elseif redis:sismember(boss..':MONSHA_BOT:'..ChatID,UserID) then 
...
if cmd == "bio" then
return sendMsg(ChatID,MsgID,"• البايو   ⇦"..biouser.."")
end
...
if cmd == "biouser" then
return SendMention(ChatID,UserID,MsgID,"• البايو   ⇦`「 "..biouser.."」`",37,USERCAR)
end
if cmd == "Upmonsh" then
...
elseif redis:sismember(boss..':MATH_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"*•* لا يمكنك كتم Myth 🎖\n") 
elseif redis:sismember(boss..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"*•* لا يمكنك كتم Myth\n") 
elseif redis:sismember(boss..':MONSHA_BOT:'..ChatID,UserID) then 
...
if UserID == 1605449126 or UserID == 1605449126 then return sendMsg(ChatID,MsgID," *•* لا يمكنك تنفيذ الامر ضد Ace 🎖 \n") end
...
if redis:sismember(boss..':MATH_BOT:',UserID) then 
DonisDown = DonisDown.."•  تم تنزيله من Myth 🎖  \n"
end 
if redis:sismember(boss..':SUDO_BOT:',UserID) then 
DonisDown = DonisDown.."•  تم تنزيله من Myth  \n"
end 
...
return sendMsg(msg.chat_id_,1,'ــــ\n'..list_settings..eueuf..'\n')
end
...
list_settings = list_settings:gsub('true', ' مقفول ')
list_settings = list_settings:gsub('false', ' مفتوح ')
return sendMsg(msg.chat_id_, msg.id_,'ـــــ\n'..list_settings..'\n')
end
...
list_settings = list_settings:gsub('true', ' مقفول ')
list_settings = list_settings:gsub('false', ' مفتوح ')
return sendMsg(msg.chat_id_,msg.id_,'ــــ\n'..list_settings..'\n')
end
